...
class SearchScreen extends React.Component {
    ...
    fetchMarkerData(){
        ...
        navigator.geolocation.getCurrentPosition((position) => {
            const barUrl = 'https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=' 
                            + position.coords.latitude 
                            + ',' 
                            + position.coords.longitude 
                            + '&radius=5000&types=bar&key=' 
                            + API_KEY;
            const clubUrl = 'https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=' 
                            + position.coords.latitude 
                            + ',' 
                            + position.coords.longitude
        ...
        //=============================================================
        fetch(clubUrl).then((data) => data.json()).then((res) => {
        ...
        //=============================================================
            fetch(barUrl).then((data) => data.json()).then((res) => {
                markers=[];
                for(var i=0; i < res.results.length; i++) {
                    var instance = res.results[i];
                                    ...
                                    <Row style={{ backgroundColor: '#1c2e63'}} size={1}>
                                        <Grid>
                                            <Row style={{ backgroundColor: '#030e2c'}} size={20}>
                                                <Left>
                                                    <Text numberOfLines={1} style={{paddingLeft: 5, fontSize: 20}}>{this.state.selectedName}</Text>
                                                </Left>
                                                <Right>
                                                    <Text style={{paddingRight: 5, fontSize: 20}}>{this.state.selectedRating}</Text>
                                                </Right>
                                            </Row>
                                            <Row style={{ backgroundColor: '#19233F'}} size={80}>
                                                <Col style={{ backgroundColor: '#19233F'}} size={60}>
                                                    <Text style={{paddingLeft: 5, marginTop: 10, fontSize: 15, backgroundColor: '#030e2c'}}>{this.state.selectedOpen}</Text>
                                                </Col>
                                                <Col style={{ backgroundColor: '#19233F'}} size={40}>
                                                    <Body>
                                                        <Button style={{marginTop: 80}}success onPress={() => this.handleRoute()}>
                                                            <Text style={{color: '#FFFFFF', fontStyle: 'normal'}}>    Add    </Text>
                                                        </Button>
                                                    </Body>                                               
                                                </Col>
                                            </Row>
